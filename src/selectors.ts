...
export const commonTableShape: TableShape = [
	[
		TableSection.name,
		...new LabeledSection(TableSection.uri, "URI:"),
		TableSection.example
	],
...
export const name = ($: CheerioSelector, $el: Cheerio) => {
	const $name = makeTableSelector(propertyTableShape, TableSection.name)(
		$,
		$el
	);
	return $name.text();
...
export const notes = ($: CheerioSelector, $el: Cheerio) => {
	const $notes = makeTableSelector(commonTableShape, TableSection.notes)(
		$,
		$el
	);
...
export const activityTypeSubClassOf = (
	...
	const $extends = makeTableSelector(
		activityTypeTableShape,
	...
	const extendsAbsoluteUrl = extendsHref && urlm.resolve(baseUrl, extendsHref);
	const subClassOf = {
		name: subClassOfName,
		type: "Link" as "Link",
...
export const id = ($: CheerioSelector, $el: Cheerio) => {
	const $uri = makeTableSelector(activityTypeTableShape, TableSection.uri)(
		$,
		$el
	);
	const id = $uri.text();
...
export const example = ($: CheerioSelector, $el: Cheerio, baseUrl: string) => {
	const examples = makeTableSelector(commonTableShape, TableSection.example)(
		$,
		$el
	)
		.find("*[id]")
...
export const url = ($: CheerioSelector, $el: Cheerio) => {
	const anchorName = makeTableSelector(commonTableShape, TableSection.name)(
		$,
		$el
	)
		.find("dfn")
...
const makeDomainOrRangeSelector = (selectDomainOrRange: Selector) => (
	...
	const as2TypeLinks = selectDomainOrRange($, $el)
		.find("code")
		.toArray()
		.map(as2TypeEl => {
...
export const disjoinWith = (
	...
	const disjointWith = $(
		makeTableSelector(coreTypeTableShape, TableSection.disjointWith)($, $el)
	)
		.find("code")
		.toArray()
		.map(el => {
...
export const subPropertyOf = ($: CheerioSelector, $el: Cheerio) => {
	const subPropertyOf = $(
		makeTableSelector(propertyTableShape, TableSection.subPropertyOf)($, $el)
	)
		.find("code")
		.toArray()
		.map(el => {
...
export const propertyTypes = ($: CheerioSelector, $el: Cheerio) => {
	const collapseEmpty = <T>(arr: T[]): T[] | void =>
