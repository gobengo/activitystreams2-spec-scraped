...
import {commonVocabsContext, easyForJavaScriptJsonldContext} from '../jsonld/context';
...
const compactedVocabulary = async (vocab: ScrapedVocabulary) => {
  const compacted =
      await jsonld.compact(vocab, compactedContext, defaultJsonldOptions);
  return compacted;
};
...
const main = async (
  ...
  const htmlFiles =
      fs.readdirSync(dir).filter(filename => filename.endsWith('.html'));
  await Promise.all(htmlFiles.map(htmlFilename => {
